...
function run(options) {
  ...
  child.on('exit', function (code, signal) {
    // We should not reference the child anymore.
    // Also, the user may restart the server in response to the events emitted below.
    child = null;
    ...
    if (signal === 'SIGUSR2' || code === 0) {
      ...
      bus.emit('exit');
      // if the user just restarted the process for some reason, we've nothing to do here.
      if (child) return;
      ...
      bus.emit('crash');
      // If the user just restarted the process, there's no more error to handle.
      if (child) return;
...
bus.on('restart', function () {
  if (child) {
    ...
    // the child has already exited, i.e. because we killed it or it crashed,
    // so we need to manually start it rather than relying on the 'exit' event handler
    run.restart();    
  }
});
